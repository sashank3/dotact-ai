...
def setup_logging(log_file="app.log", level=logging.INFO):
    ...
    """
    # Read logs directory from global config
    ...
    file_handler.setLevel(LOGGING_CONFIG.get("level", level))
    file_handler.setFormatter(formatter)

    console_handler.setLevel(LOGGING_CONFIG.get("level", level))
    console_handler.setFormatter(formatter)

    logger = logging.getLogger()
    logger.setLevel(LOGGING_CONFIG.get("level", level))
